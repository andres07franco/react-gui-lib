...
import  SearchLoockup from '../search-lookup/SearchLookup.jsx';
import './Form.css'
...
const Field = (props) => {
    ...
    return (
        <div  className="form-row" style={{ with: "100%", display: 'flex', flexWrap: "nowrap" }}>
            ...
            {  "autocomplete".indexOf(typeFiled)  > -1  &&
                <Autocomplete data={props.data}
                            inputClassName={`${inputClassName} ${getErrorClass(value)}`}
                            fieldText={props.fieldText}
                            onChange={(item) => onChange(item)} 
                            value={value} />
            }  
            {  "searchlookup".indexOf(typeFiled)  > -1  &&
                <SearchLoockup settings={props.settings}
                            inputClassName={`${getErrorClass(value)}`}
                            onChange={(item) => onChange(item)} 
                            value={value} />
            }            
        </div>
...
const FormRow = (props) => {
    ...
    return (<div className="dialogFormRows">{childrenWithProps}</div>)
}
...
export { Field, Form, FormRow, Button }